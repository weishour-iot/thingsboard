...
@RuleNode(
        ...
        nodeDescription = "将消息发布到 AWS SQS",
        nodeDetails = "将消息负载和元数据属性发布到 AWS SQS 队列。 出站消息将包含" +
                "响应字段（<code>messageId</code>、<code>requestId</code>、<code>messageBodyMd5</code>、<code>messageAttributesMd5</code>" +
                ", <code>sequenceNumber</code>) 在来自 AWS SQS 的消息元数据中。" +
                "例如，可以使用 <code>metadata.requestId</code> 访问 <b>requestId</b> 字段。",
